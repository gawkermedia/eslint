...
function SourceCode(text, ast) {
    ...
    this.getTokens = tokensAndCommentsStore.getTokens;

    this.attachedCommentStore = new WeakMap();
...
SourceCode.prototype = {
     ...
     */
    getComments(node) {
        if (this.attachedCommentStore.has(node)) {
            return this.attachedCommentStore.get(node);
        }

        const isComment = node => node.type === "Line" || node.type === "Block";
        const isCommentOrPunctuation = node => isComment(node) || node.type === "Punctuator";
        ...
        if (
            ...
            attachedComments.children.push(tokens.filter(isComment));
        }

        if (node.type !== "Program") {
            let currentToken = this.getTokenOrCommentBefore(node);

            while (currentToken && isCommentOrPunctuation(currentToken)) {
                if (isComment(currentToken)) {
                    attachedComments.leading.push(currentToken);
                }
            ...
            while (currentToken && isCommentOrPunctuation(currentToken)) {
                if (isComment(currentToken)) {
                    attachedComments.trailing.push(currentToken);
                }
                currentToken = this.getTokenOrCommentAfter(currentToken);
            }
        }

        this.attachedCommentStore.set(node, attachedComments);
